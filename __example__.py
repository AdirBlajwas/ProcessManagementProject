...
from reporter import EventLogReporter, ResourceScheduleReporter, DfReporter, NO_DIAGNOSIS
...
class NaivePlanner(Planner):
    def __init__(self, eventlog_file, data_columns):
        ...
        self.df_reporter  = DfReporter(data_columns)
    ...
    def report(self, case_id, element, timestamp, resource, lifecycle_state, data=None):
        ...
        self.df_reporter.callback(case_id, element, timestamp, resource, lifecycle_state, data)
...
if __name__ == '__main__':
    ...
    days = 50
    result = simulator.run(days*24)

    print(result)

    # Create graph for the actual simulation duration (10 days = 240 hours)
    planner.resource_reporter.create_graph(0, days*24)